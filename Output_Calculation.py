...
class OutputCalculation:
    ...
    def __set_up_master(self):
        ...
        assert (
            self._output_format in OutputCalculation.OUTPUT_FORMATS
        ), f"Provided output format {self._output_format} is NOT VALID, select only from implemented formats: {OutputCalculation.OUTPUT_FORMATS}"
        assert (
            self.__set_up_valid_maap_url
        ), f"Invalid URL: see assertions and/or possibly missing s3://maap-ops-workspace/shared/ in url. Provided url: {self._output_maap_url}"
        assert (
            self._day_search_range >= 0
        ), f"Invalid provided day range {self._day_search_range}. Must be x >= 0"
        assert (
            self._feds_input.crs == self._ref_input.crs
        ), f"Mismatching CRS for FEDS and reference; must correct before continuing: feds: {self._feds_input.crs} vs ref: {self._ref_input.crs}"
    ...
    def __set_up_valid_maap_url(self):
        ...
        try:
            ...
            if nested:
                ...
                assert (
                    len(bucket) == 2
                ), "FATAL: bucket + prefix design should only be len2"
                response = s3.list_objects_v2(Bucket=bucket[0], Prefix=bucket[1])
                folders = set(
                    object["Key"][: object["Key"].rfind("/") + 1]
                ...
                if bucket[1] in folders:
                    ...
                    logging.error(
                        f"ERR: prefix {bucket[1]} not located; invalid url passed {self._output_maap_url}"
                    )
                    return False
            else:
            ...
            print(
                "Could not connect to the AWS endpoint. Please check your AWS configuration."
            )
    ...
    def __run_calculations(self):
        ...
        try:
            for feds_ref_pair in index_pairs:
                # (e.g. (4, 1) <- feds index 4 best matches with ref poly at index 1

                if feds_ref_pair[1] is None:
                    # no reference polygon --> attach none to tracked calculations
                    for key in calculations:
                        if key == "index_pairs":
                            calculations[key].append(feds_ref_pair)
                        else:
                            calculations[key].append(None)
                    continue

                # else, process area calculations
                feds_poly = self._feds_input.polygons[
                    self._feds_input.polygons["index"] == feds_ref_pair[0]
                ]
                ref_poly = self._ref_input.polygons[
                    self._ref_input.polygons["index"] == feds_ref_pair[1]
                ]
                # submit to Dask cluster
                futures.append(
                    client.submit(
                        OutputCalculation.pair_stats_calculations,
                        feds_poly,
                        ref_poly,
                        feds_ref_pair,
                    )
        ...
        # aggregate individual dicts into calculations
        for r in results:
            calculations["index_pairs"].append(r["index_pair"])
            calculations["ratio"].append(r["ratio"])
            calculations["accuracy"].append(r["accuracy"])
            calculations["precision"].append(r["precision"])
            calculations["recall"].append(r["recall"])
            calculations["iou"].append(r["iou"])
            calculations["f1"].append(r["f1"])
            calculations["symm_ratio"].append(r["symm_ratio"])

        # verify same sizing
        for key in calculations:
            assert len(calculations[key]) == len(
                index_pairs
            ), f"FATAL: mismatching sizing of arr at key {key} for calculations"
    ...
    def __print_output(self):
        ...
        for i in range(len(calculations["index_pairs"])):
            ...
            if all(value is None for value in vals):
                print(
                    f'NO CALCULATION RESULTS, SKIP FEDS INDEX {calculations["index_pairs"][i][0]} & REFERENCE INDEX {calculations["index_pairs"][i][0]}'
                )
            else:
                print(
                    f'CALCULATED A RESULT: POLYGON FEDS AT INDEX {calculations["index_pairs"][i][0]} AGAINST REFERENCE POLYGON AT INDEX {calculations["index_pairs"][i][1]}:'
                )
                print(
                    f"Ratio: {vals[0]}, Accuracy: {vals[1]}, Precision: {vals[2]}, Recall: {vals[3]}, IOU: {vals[4]}, F1 {vals[5]}, Symmetric Ratio: {vals[6]}"
                )
                print(
                    f"All measurements in units {self._feds_input.polygons.crs.axis_info[0].unit_name}"
                )
    ...
    def __plot_output(self):
        ...
        for pair in calculations["index_pairs"]:
            ...
            feds_poly = feds_polygons[
                feds_polygons["index"] == calculations["index_pairs"][i][0]
            ]
            ref_poly = ref_polygons[
                ref_polygons["index"] == calculations["index_pairs"][i][1]
            ]
            ...
            feds_plot = feds_poly.plot(
                ax=ax,
                color="red",
                edgecolor="black",
                linewidth=1.5,
                label="FEDS Fire Estimate",
            )
            ref_plot = ref_poly.plot(
                ax=ax,
                color="gold",
                edgecolor="black",
                linewidth=1.5,
                hatch="\\",
                alpha=0.7,
                label="Reference Nearest Date + Intersection",
            )
            ...
            # show plot
            ax.set_title(
                f"FEDS ({index1}) at {feds_time} VS. Ref ({index2}) at {ref_time}",
                fontsize=17,
            )
    ...
    def get_nearest_by_date(dataset, timestamp, dayrange: int):
        ...
        clos_dict = {
            abs(timestamp.timestamp() - date.timestamp()): date for date in transformed
        }
        ...
        if not abs(timestamp.day - res.day) <= dayrange:
            logging.error(
                "WARNING in get_nearest_by_date: No dates found in specified range for this reference collection; try a more flexible range by adjusting `dayrange` var"
            )
    ...
    def closest_date_match(self, client) -> list:
        """
        Given feds and reference polygons, return a list
        mapping the feds inputs to any nearby reference polygons.
        The reference polygon may be None if no match is found.

        [(feds_poly_index, ref_poly_index), (feds_poly_index, None)]
        """

        logging.info(
            f"Number of total feds_polygons: {len(self._feds_input.polygons.index)}"
        )
        logging.info(
            f"Number of total ref_polygons: {len(self._ref_input.polygons.index)}"
        )

        total_iterations = self._feds_input.polygons.shape[0]
        day_search_range = self._day_search_range

        # broadcast=True copies full gdf into memory on each worker;
        # reduces network overhead during computation, but needs each worker to
        # have sufficient memory
        feds_polygons = client.scatter(self._feds_input.polygons, broadcast=True)
        ref_polygons = client.scatter(self._ref_input.polygons, broadcast=True)

        # Submit tasks to Dask workers for parallel processing.
        # searching for reference polygon matches for one feds polygon == one task
        try:
            futures = [
                client.submit(
                    OutputCalculation.match_one_feds_poly,
                    feds_polygons,
                    i,
                    ref_polygons,
                    day_search_range,
    ...
    def tif_analysis(self, tif_path: str, req_calc: str, date_restrict=None):
        ...
        assert (
            req_calc in valid_calc_choices
        ), f"Provided req_calc argument is not valid, please select a choice from the following: {valid_calc_choices}"
        ...
        for pair in indices:
            ...
            if date_restrict is not None:
                assert (
                    type(date_restrict) is int
                ), "date_restrict must be int, as in number of days permitted for timestamp difference"
                # assert of datetime type
                feds_time = datetime.strptime(
                    feds_inst.t.values[0], "%Y-%m-%dT%H:%M:%S"
                )
                is_within_time_given = OutputCalculation.get_nearest_by_date(
                    ref_inst, feds_time, date_restrict
                )
    ...
    def transform_polygon(self, geoseries):
        ...
        transformer = pyproj.Transformer.from_crs(
            mercator_crs, nad83_crs, always_xy=True
        )
    ...
    def write_to_csv(self):
        ...
        with open(file_name, "w", newline="") as csvfile:
            ...
            for i in range(num_rows):
                ...
                if all(
                    ...
                    # poly extraction
                    print(
                        f"VERBOSE: BOUNDS OF ALL FEDS SET: {feds_polygons.total_bounds}"
                    )
                    ...
                    minx, miny, maxx, maxy = diff_area.to_crs(
                        evt20.rio.crs
                    ).total_bounds
                    ...
                    row_data = {
                        ...
                        "feds_polygon": self._feds_input._polygons.loc[
                            calculations["index_pairs"][i][0]
                        ]["geometry"].wkt,
                        "ref_index": calculations["index_pairs"][i][1],
                        "ref_polygon": self._ref_input._polygons.loc[
                            calculations["index_pairs"][i][1]
                        ]["geometry"].wkt,
        ...
        print("\n")
        print(
            f"CSV output complete! Check file {file_name} for results. NOTE: None result rows were excluded."
        )
    ...
    def truePos(feds_inst, nifc_inst):
        ...
        result = OutputCalculation.areaCalculation(
            overlay
        )  # overlay.geometry.area.item()
        return result
    ...
    def falseNeg(feds_inst, nifc_inst):
        ...
        result = gpd.overlay(
            feds_neg, nifc_inst, keep_geom_type=False, how="intersection"
        )
    ...
    def falsePos(feds_inst, nifc_inst):
        ...
        result = gpd.overlay(
            nifc_neg, feds_inst, keep_geom_type=False, how="intersection"
        )
        result = OutputCalculation.areaCalculation(result)
    ...
    def trueNeg(feds_inst, nifc_inst):
        ...
        inter_neg = gpd.overlay(
            feds_neg, nifc_neg, keep_geom_type=False, how="intersection"
        )
    ...
    def ratioCalculation(feds_inst, nifc_inst):
        ...
        assert (
            feds_area is not None
        ), "None type detected for area; something went wrong"
        assert (
            nifc_area is not None
        ), "None type detected for area; something went wrong"
    ...
    def precisionCalculation(feds_inst, nifc_inst):
        ...
        """
        assert isinstance(feds_inst, pd.DataFrame) and isinstance(
            nifc_inst, pd.DataFrame
        ), "Object types will fail intersection calculation; check inputs"
    ...
    def IOUCalculation(feds_inst, nifc_inst):
        ...
        # overlay = gpd.overlay(feds_inst, nifc_inst, how='intersection')
        TP = OutputCalculation.truePos(feds_inst, nifc_inst)
        FP = OutputCalculation.falsePos(
            feds_inst, nifc_inst
        )  # feds + nifc agree on no burning
        FN = OutputCalculation.falseNeg(
            feds_inst, nifc_inst
        )  # feds thinks unburned when nifc burned
    ...
    def symmDiffRatioCalculation(feds_inst, nifc_inst):
        ...
        assert (
            sym_diff.shape[0] == 1
        ), "Multiple sym_diff entries identified; pair accuracy evaluation will fail."
    ...
    @staticmethod
    def pair_stats_calculations(feds_poly, ref_poly, feds_ref_pair) -> dict:
        """
        Given one feds poly and one ref poly, orchestrates calculations for
        ...
        """
        calculations = {}
        calculations["index_pair"] = feds_ref_pair
        # run through calculations
        calculations["ratio"] = OutputCalculation.ratioCalculation(feds_poly, ref_poly)
        calculations["accuracy"] = OutputCalculation.accuracyCalculation(
            feds_poly, ref_poly
        )
        calculations["precision"] = OutputCalculation.precisionCalculation(
            feds_poly, ref_poly
        )
        calculations["recall"] = OutputCalculation.recallCalculation(
            feds_poly, ref_poly
        )
        calculations["iou"] = OutputCalculation.IOUCalculation(feds_poly, ref_poly)
        calculations["f1"] = OutputCalculation.f1ScoreCalculation(feds_poly, ref_poly)
        calculations["symm_ratio"] = OutputCalculation.symmDiffRatioCalculation(
            feds_poly, ref_poly
        )  # indep calc
    ...
    def match_one_feds_poly(
        feds_polygons, feds_poly_i: int, ref_polygons, day_search_range: int
    ) -> list:
        """
        Given one feds polygon and a geodataframe of reference polygons,
        return possible matches with reference polygons.

        feds_polygons: geodataframe of feds polygons
        feds_poly_i: integer index of specific feds polygon to match
        ref_polygons: geodataframe of reference polygons
        day_search_range: if difference between feds and ref polygon
        exceeds this int, warning will be given.
        ...
        Note: must be static method so Dask will not try to serialize
        the class each time and send it with every function call.
        """
        feds_poly = feds_polygons.iloc[[feds_poly_i]]
        ...
        if time_matches is None:
            sys.stdout.write(
                f"TIME MATCH WARNING: the intersecting pair does not have a timestamp difference within the specified day search range window: {day_search_range} \n"
            )
            sys.stdout.flush()
            sys.stdout.write(
